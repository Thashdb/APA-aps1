...
int partition(int A[], int inicio, int fim){
    ...
    // Escolhe índice aleatório no intervalo [inicio, fim]
    int indicePivo = inicio + rand() % (fim - inicio + 1);

    // Coloca o pivô sorteado na posição inicial
    swap(&A[inicio], &A[indicePivo]);
...
int main() {
    ...
    printf("===== Quick Sort (Hoare) - aleatório (%d elementos) =====\n", n);
